...
| **Creative output quality** | **3.5** | **~6 at the scene level (NOT a verified finished cut)** | Fonts actually load (Fraunces/Inter); native data-viz; legible scrimmed captions/over-image text; relevant sourcing; brand chips; big-number/timeline layouts — all confirmed in clean per-scene draft renders. Caveat: no full upgraded `video.mp4` completed (auto-gate holds on one layout edge), so this is a per-scene/capability score, not a finished-film grade. |
...
**I did NOT complete 5 fully-rendered videos.** Driving the real upgraded pipeline surfaced a *cascade* of last-mile render bugs (TTS oversubscription → transient-LLM death → over-image contrast → over-strict contrast gate → caption/title occlusion → big-number overflow → big-number+brand-chip overlap). I fixed **8** of them (all committed, tested or live-verified). The pipeline now reaches compose reliably and produces **5 of 6 structurally-clean scenes** per video — a large jump from the pre-fix amateur cut — but the deterministic auto-gate (correctly) requires *all* scenes clean, and one LLM-generated layout combination (big-number + brand chip in one scene) still blocks the full concatenated render. **The only complete `video.mp4` is the §3 pre-fix "coffee-vs-tea" render — the "before."** The capability climb is real and visible in per-scene draft renders + artifacts; the finished-cut climb is not yet demonstrable end-to-end. The single highest-leverage next task is a layout-combination grid (guarantee zero overlap when two content blocks stack), after which the ladder should complete — the reliability blockers are already cleared.
...
✅ Real bugs fixed with tests — **485 green, 0 dead** (was 420 + 16 dead). ✅ Spine still deterministic & contract-valid. ✅ Both gates still behave (un-approvable `block` unit-tested 5 ways; gates exercised live in §3, including a real end-to-end render there). ✅ Docs reconciled. ✅ Fleet taught new craft; vocabulary extended in lockstep with bundled OFL fonts. ❌ **Five completed videos: NOT done.** The upgraded pipeline now produces 5/6 gate-clean scenes per video (huge improvement) but a remaining layout-combination edge case + the correctly-strict all-scenes auto-gate block the finished cut. Reported truthfully rather than faked. The reliability + rendering fixes are real and committed; finishing the ladder needs one more focused layout-robustness pass.
