"""Flux's brain — one swappable chat() function, plus a converse() chat seam.
...
constant below (or set the FLUX_LLM env var). Everything else is provider-agnostic.
...
- "gemini"   — Google Gemini (free tier). Activate with FLUX_LLM=gemini and a
...
- "deepseek" — DeepSeek (OpenAI-compatible, raw requests). Activate with
  FLUX_LLM=deepseek and a DEEPSEEK_API_KEY in .env. Wired but untested (no key
...
NOTE ON CHAT: the co-worker REPL ("Talk to Flux", chat.py) does NOT use chat().
...
# ======================================================================
# Default (unset) = "claude" (subscription). Export FLUX_LLM=gemini / deepseek to swap.
PROVIDER = os.environ.get("FLUX_LLM", "claude").strip().lower()

# Per-use Claude models (subscription). Coaching wants strong, concise craft
# reasoning; chat is fine on a fast Sonnet-class brain and keeps the heavier rate
# limit free.
CLAUDE_MODEL = "claude-sonnet-4-6"     # coaching brain — Claude Sonnet 4.6 (full slug; no bare aliases)
...
# Alternative-provider models (only used when FLUX_LLM selects them).
    ...
    if _genai_model is None:
        ...
        if not key:
            raise RuntimeError(
                ...
                "https://aistudio.google.com -> Get API key), or unset FLUX_LLM to "
...
def _chat_deepseek(system: str, user: str) -> str:
    ...
    if not key:
        raise RuntimeError(
            ...
            "-> API keys), or unset FLUX_LLM to use the default Claude subscription brain."
        )
    r = requests.post(
...
# ======================================================================
# MULTI-TURN SEAM — converse() for "Talk to Flux" (chat.py)
# ======================================================================
...
# survives a future brain swap. The optional `flux` wiring is the tool+approval
# bundle; pass flux=None to disable the native-tool job (marker fallback handles it).


def _render_turns(recent_turns) -> str:
    label = {"user": "User", "flux": "Flux"}
    return "\n".join(
...
async def _converse_async(system, summary, recent_turns, user_msg, flux, model) -> str:
    from claude_agent_sdk import query, ClaudeAgentOptions
    from claude_agent_sdk.types import AssistantMessage, TextBlock, ResultMessage
    ...
    if flux is not None:
        ...
        opts_kwargs["mcp_servers"] = {"flux": flux["server"]}
        opts_kwargs["can_use_tool"] = flux["can_use_tool"]
...
def converse(system: str, summary: str, recent_turns, user_msg: str,
             *, flux=None, model: str | None = None) -> str:
    """Multi-turn chat seam (Claude SDK). Returns Flux's reply text.
    ...
    recent_turns  : list of {"role": "user"|"flux", "content": str} — the window.
    user_msg      : the user's new message.
    flux          : optional tool wiring {"server", "can_use_tool"}; None disables
                    mid-chat scripting via the native tool.
    """
    _warn_if_metered()
    try:
        return asyncio.run(asyncio.wait_for(
            _converse_async(system, summary, recent_turns, user_msg, flux, model),
            CHAT_TIMEOUT_SEC))
