...
(chat.distill_and_save), which is what persists Flux's durable memory at session
...
COMPACTION_SYSTEM = (
    "You compress a conversation into a durable memo about the USER, for a "
    "production-coach co-worker who must remember them across sessions."
)
...
def _compaction_user_prompt(existing_summary: str, turns: list[dict[str, str]]) -> str:
    ...
    return (
        ...
        "kinds of videos and channel they make, the craft specialists they work "
        "with, their taste and standards (pacing, motion energy, restraint, "
        "loudness, AV coherence), decisions made, and coaching that landed. DISCARD "
        "greetings, small talk, pleasantries, and transient chatter. Merge with what "
        "you already knew; don't repeat. Output ONLY the updated memo as concise "
        "prose — no preamble, no commentary."
    )
