"""Durable, provider-agnostic state for Cadence — the source of truth on disk.

Two things live through this module:

1. Cadence's chat memory ("Talk to Cadence"). Across sessions, Cadence's ONLY long-term
...
2. The audio engine's memory.json (a log of past mix runs), through the same
...
def append_turn(state: dict[str, Any], role: str, content: str) -> None:
    """Append one turn. role is 'user' or 'cadence' (kept provider-neutral)."""
