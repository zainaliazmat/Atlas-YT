...
(chat.distill_and_save), which is what persists Cadence's durable memory at session
...
COMPACTION_SYSTEM = (
    "You compress a conversation into a durable memo about the USER, for an audio / "
    "sound-design co-worker who must remember them across sessions."
)
...
def _compaction_user_prompt(existing_summary: str, turns: list[dict[str, str]]) -> str:
    ...
    return (
        ...
        f"{convo}\n\n"
        "Update the memo. KEEP only durable, craft-improving facts: the kinds of "
        "videos and topics they make, their channel and audience, their voice / "
        "narration preferences (the Kokoro voice, pace), the music moods and beds "
        "that have worked, their loudness/ducking posture, and rulings made on what "
        "audio clears. DISCARD greetings, small talk, pleasantries, and transient "
        "chatter. Merge with what you already knew; don't repeat. Output ONLY the "
        "updated memo as concise prose — no preamble, no commentary."
    )
