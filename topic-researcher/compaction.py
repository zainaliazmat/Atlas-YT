...
(chat.distill_and_save), which is what persists Sage's durable memory at session
...
COMPACTION_SYSTEM = (
    "You compress a conversation into a durable memo about the USER, for a research "
    "co-worker who must remember them across sessions."
)
...
def _compaction_user_prompt(existing_summary: str, turns: list[dict[str, str]]) -> str:
    ...
    return (
        ...
        "topics the user researches, their interests and standards (e.g. how "
        "skeptical/rigorous they want sources to be), decisions made, and useful "
        "findings established. DISCARD greetings, small talk, pleasantries, and "
        "transient chatter. Merge with what you already knew; don't repeat. Output "
        "ONLY the updated memo as concise prose — no preamble, no commentary."
    )
