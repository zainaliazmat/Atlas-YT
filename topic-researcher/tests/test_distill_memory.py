...
check (real chat with junk + facts -> /exit -> relaunch -> "what do you know about
me"). Only the plumbing is unit-tested here.
"""
import json
import pathlib
import sys
...
def test_summary_command_distills_and_shows(monkeypatch, capsys=None):
    with tempfile.TemporaryDirectory() as d:
        monkeypatch.setattr(chat, "STATE_PATH", pathlib.Path(d) / "chat_state.json")
        calls = []
        state = _state("s", [{"role": "user", "content": "a"}])
...
def test_recover_pending_folds_in_next_launch(monkeypatch):
    with tempfile.TemporaryDirectory() as d:
        sp = pathlib.Path(d) / "chat_state.json"
        monkeypatch.setattr(chat, "STATE_PATH", sp)
        # Simulate a prior failed distill: a file with pending turns.
