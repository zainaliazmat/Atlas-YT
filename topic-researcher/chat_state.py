"""Durable, provider-agnostic state for Sage — the source of truth on disk.

Two things live through this module:

1. Sage's chat memory ("Talk to Sage", Phase 2). Across sessions, Sage's ONLY
   long-term memory is a single distilled SUMMARY. The raw transcript is NOT
   persisted between sessions — it lives only in RAM for the duration of a session
   and is distilled into the summary on session end (see chat.distill_and_save).
...
2. The research engine's memory.json (a log of past research runs), through the
...
def append_turn(state: dict[str, Any], role: str, content: str) -> None:
    """Append one turn. role is 'user' or 'sage' (kept provider-neutral)."""
