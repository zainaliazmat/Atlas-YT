"""Durable, provider-agnostic state for Magpie — the source of truth on disk.

Two things live through this module:

1. Magpie's chat memory ("Talk to Magpie"). Across sessions, Magpie's ONLY long-term
...
2. The source engine's memory.json (a log of past sourcing runs), through the same
   atomic_write_json / load_json helpers.
...
def append_turn(state: dict[str, Any], role: str, content: str) -> None:
    """Append one turn. role is 'user' or 'magpie' (kept provider-neutral)."""
