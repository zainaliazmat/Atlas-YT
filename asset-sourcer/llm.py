"""Magpie's brain — one swappable chat() function, plus a converse() chat seam.

Magpie's ENGINE is deterministic and does NOT call an LLM — sourcing and license
clearance are retrieval + a truth table, not taste. This module exists for the
co-worker REPL ("Talk to Magpie", chat.py) and the cross-session memory distiller,
which are the only places a brain is used.

`chat(system, user) -> str` is the ONLY text-completion call the distiller makes.
To swap LLM providers you change ONE place: the `PROVIDER` constant below (or set
the MAGPIE_LLM env var). Everything else is provider-agnostic.
...
- "gemini"   — Google Gemini (free tier). Activate with MAGPIE_LLM=gemini and a
...
- "deepseek" — DeepSeek (OpenAI-compatible, raw requests). Activate with
  MAGPIE_LLM=deepseek and a DEEPSEEK_API_KEY in .env.
...
# ======================================================================
# Default (unset) = "claude" (subscription). Export MAGPIE_LLM=gemini / deepseek to swap.
PROVIDER = os.environ.get("MAGPIE_LLM", "claude").strip().lower()

# Per-use Claude models (subscription). The distiller is a light summarization job;
# chat is fine on a fast Sonnet-class brain and keeps the heavier rate limit free.
CLAUDE_MODEL = "claude-sonnet-4-6"     # distiller brain ("haiku" spends even less)
...
# Alternative-provider models (only used when MAGPIE_LLM selects them).
    ...
    if _genai_model is None:
        ...
        if not key:
            raise RuntimeError(
                ...
                "https://aistudio.google.com -> Get API key), or unset MAGPIE_LLM to "
...
def _chat_deepseek(system: str, user: str) -> str:
    ...
    if not key:
        raise RuntimeError(
            ...
            "-> API keys), or unset MAGPIE_LLM to use the default Claude subscription brain."
        )
    r = requests.post(
...
def chat(system: str, user: str) -> str:
    ...
    The single seam the distiller talks through. Provider is chosen by PROVIDER (one
    place), defaulting to Claude. Keep the signature (system, user) -> str identical
    across providers so it stays a true drop-in swap.
    """
...
# ======================================================================
# MULTI-TURN SEAM — converse() for "Talk to Magpie" (chat.py)
# ======================================================================
...
# survives a future brain swap. The optional `magpie` wiring is the tool+approval
# bundle; pass magpie=None to disable the native-tool job (marker fallback handles it).
...
async def _converse_async(system, summary, recent_turns, user_msg, magpie, model) -> str:
    from claude_agent_sdk import query, ClaudeAgentOptions
    from claude_agent_sdk.types import AssistantMessage, TextBlock, ResultMessage
    ...
    if magpie is not None:
        ...
        opts_kwargs["mcp_servers"] = {"magpie": magpie["server"]}
        opts_kwargs["can_use_tool"] = magpie["can_use_tool"]
...
def converse(system: str, summary: str, recent_turns, user_msg: str,
             *, magpie=None, model: str | None = None) -> str:
    """Multi-turn chat seam (Claude SDK). Returns Magpie's reply text.
    ...
    recent_turns  : list of {"role": "user"|"magpie", "content": str} — the window.
    user_msg      : the user's new message.
    magpie        : optional tool wiring {"server", "can_use_tool"}; None disables
                    mid-chat jobs via the native tools.
    """
    _warn_if_metered()
    try:
        return asyncio.run(asyncio.wait_for(
            _converse_async(system, summary, recent_turns, user_msg, magpie, model),
            CHAT_TIMEOUT_SEC))
