...
(chat.distill_and_save), which is what persists Vera's durable memory at session
...
COMPACTION_SYSTEM = (
    "You compress a conversation into a durable memo about the USER, for a reference "
    "analyst who must remember them across sessions."
)
...
def _compaction_user_prompt(existing_summary: str, turns: list[dict[str, str]]) -> str:
    ...
    return (
        ...
        f"{convo}\n\n"
        "Update the memo. KEEP only durable, taste-improving facts: the look and feel "
        "the user is chasing, the references they admire, their standards (how snappy, "
        "how loud, how kinetic), decisions made, and the style targets settled. DISCARD "
        "greetings, small talk, pleasantries, and transient chatter. Merge with what "
        "you already knew; don't repeat. Output ONLY the updated memo as concise prose "
