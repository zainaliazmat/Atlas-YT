...
(chat.distill_and_save), which is what persists Mason's durable memory at session
...
COMPACTION_SYSTEM = (
    ...
    "composition-engineer co-worker who must remember them across sessions."
)
...
def _compaction_user_prompt(existing_summary: str, turns: list[dict[str, str]]) -> str:
    ...
    return (
        ...
        "kinds of videos they make, recurring determinism gotchas and render "
        "quirks worth not relearning, stutter step-count and effect choices that "
        "landed, and decisions made. DISCARD greetings, small talk, pleasantries, "
        "and transient chatter. Merge with what you already knew; don't repeat. "
        "Output ONLY the updated memo as concise prose — no preamble, no commentary."
    )
