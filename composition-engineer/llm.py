"""Mason's brain — one swappable chat() function, plus a converse() chat seam.

`chat(system, user) -> str` is the ONLY text-completion call the chat persona and
the distiller make. (Mason's COMPOSITION engine is pure, deterministic code — it
calls no LLM. The brain is only for the co-worker chat + the memory distill.) To
swap LLM providers you change ONE place: the `PROVIDER` constant below (or set the
MASON_LLM env var). Everything else is provider-agnostic.
...
- "gemini"   — Google Gemini (free tier). Activate with MASON_LLM=gemini and a
...
- "deepseek" — DeepSeek (OpenAI-compatible, raw requests). Activate with
  MASON_LLM=deepseek and a DEEPSEEK_API_KEY in .env. Wired but untested (no key
...
NOTE ON CHAT: the co-worker REPL ("Talk to Mason", chat.py) does NOT use chat().
...
prompt. We use the SDK's query() — a fresh, isolated context per call — rather than
reusing one ClaudeSDKClient session, which would carry a single accumulating
...
# ======================================================================
# Default (unset) = "claude" (subscription). Export MASON_LLM=gemini / deepseek to swap.
PROVIDER = os.environ.get("MASON_LLM", "claude").strip().lower()

# Per-use Claude models (subscription). Mason's engine is pure code; the brain is
# only the chat seam + the memory distill, both fine on a fast Sonnet-class model.
CLAUDE_MODEL = "claude-sonnet-4-6"     # distill/compaction brain
...
# Alternative-provider models (only used when MASON_LLM selects them).
    ...
    if _genai_model is None:
        ...
        if not key:
            raise RuntimeError(
                ...
                "https://aistudio.google.com -> Get API key), or unset MASON_LLM to "
                "use the default Claude subscription brain."
            )
        genai.configure(api_key=key)
    # system_instruction is bound at construction; rebuild each call since distinct
...
def _chat_deepseek(system: str, user: str) -> str:
    ...
    if not key:
        raise RuntimeError(
            ...
            "-> API keys), or unset MASON_LLM to use the default Claude subscription brain."
        )
    r = requests.post(
...
def chat(system: str, user: str) -> str:
    ...
    This is the single seam the chat persona and distiller talk through. Provider is
...
# ======================================================================
# MULTI-TURN SEAM — converse() for "Talk to Mason" (chat.py)
# ======================================================================
...
# survives a future brain swap. The optional `wiring` is the tool+approval bundle;
# pass wiring=None to disable the native-tool job (marker fallback handles it).


def _render_turns(recent_turns) -> str:
    label = {"user": "User", "mason": "Mason"}
    return "\n".join(
...
async def _converse_async(system, summary, recent_turns, user_msg, wiring, model) -> str:
    from claude_agent_sdk import query, ClaudeAgentOptions
    from claude_agent_sdk.types import AssistantMessage, TextBlock, ResultMessage
    ...
    if wiring is not None:
        ...
        opts_kwargs["mcp_servers"] = {"mason": wiring["server"]}
        opts_kwargs["can_use_tool"] = wiring["can_use_tool"]
...
def converse(system: str, summary: str, recent_turns, user_msg: str,
             *, wiring=None, model: str | None = None) -> str:
    """Multi-turn chat seam (Claude SDK). Returns Mason's reply text.
    ...
    recent_turns  : list of {"role": "user"|"mason", "content": str} — the window.
    user_msg      : the user's new message.
    wiring        : optional tool wiring {"server", "can_use_tool"}; None disables
                    mid-chat jobs via the native tools.
    """
    _warn_if_metered()
    try:
        return asyncio.run(asyncio.wait_for(
            _converse_async(system, summary, recent_turns, user_msg, wiring, model),
            CHAT_TIMEOUT_SEC))
    except asyncio.TimeoutError:
        # A chat turn that exceeds this isn't "thinking" — it's stalled. The most
        # common cause is the Claude subscription's rolling rate-limit, or a network
        # stall. Surface it instead of hanging forever; the REPL stays retryable.
        raise RuntimeError(
