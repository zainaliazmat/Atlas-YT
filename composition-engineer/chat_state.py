"""Durable, provider-agnostic state for Mason — the source of truth on disk.

Two things live through this module:

1. Mason's chat memory ("Talk to Mason"). Across sessions, Mason's ONLY long-term
   memory is a single distilled SUMMARY. The raw transcript is NOT persisted between
   sessions — it lives only in RAM for the duration of a session and is distilled into
   the summary on session end (see chat.distill_and_save).
   Shape on disk (chat_state.json):

       {
           "summary":  "<distilled durable context about the collaborator>",
...
2. The composition engine's memory.json (a log of past composition runs), through the
   same atomic_write_json / load_json helpers.
...
def append_turn(state: dict[str, Any], role: str, content: str) -> None:
    """Append one turn. role is 'user' or 'mason' (kept provider-neutral)."""
