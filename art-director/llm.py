"""Iris's brain — one swappable chat() function, plus a converse() chat seam.
...
constant below (or set the IRIS_LLM env var). Everything else is provider-agnostic.
...
- "gemini"   — Google Gemini (free tier). Activate with IRIS_LLM=gemini and a
...
- "deepseek" — DeepSeek (OpenAI-compatible, raw requests). Activate with
  IRIS_LLM=deepseek and a DEEPSEEK_API_KEY in .env. Wired but untested (no key
...
NOTE ON CHAT: the co-worker REPL ("Talk to Iris", chat.py) does NOT use chat().
...
# ======================================================================
# Default (unset) = "claude" (subscription). Export IRIS_LLM=gemini / deepseek to swap.
PROVIDER = os.environ.get("IRIS_LLM", "claude").strip().lower()
...
# Alternative-provider models (only used when IRIS_LLM selects them).
    ...
    if _genai_model is None:
        ...
        if not key:
            raise RuntimeError(
                ...
                "https://aistudio.google.com -> Get API key), or unset IRIS_LLM to "
...
def _chat_deepseek(system: str, user: str) -> str:
    ...
    if not key:
        raise RuntimeError(
            ...
            "-> API keys), or unset IRIS_LLM to use the default Claude subscription brain."
        )
    r = requests.post(
...
# ======================================================================
# MULTI-TURN SEAM — converse() for "Talk to Iris" (chat.py)
# ======================================================================
...
# survives a future brain swap. The optional `iris` wiring is the tool+approval
# bundle; pass iris=None to disable the native-tool job (marker fallback handles it).


def _render_turns(recent_turns) -> str:
    label = {"user": "User", "iris": "Iris"}
    return "\n".join(
...
async def _converse_async(system, summary, recent_turns, user_msg, iris, model) -> str:
    from claude_agent_sdk import query, ClaudeAgentOptions
    from claude_agent_sdk.types import AssistantMessage, TextBlock, ResultMessage
    ...
    if iris is not None:
        ...
        opts_kwargs["mcp_servers"] = {"iris": iris["server"]}
        opts_kwargs["can_use_tool"] = iris["can_use_tool"]
...
def converse(system: str, summary: str, recent_turns, user_msg: str,
             *, iris=None, model: str | None = None) -> str:
    """Multi-turn chat seam (Claude SDK). Returns Iris's reply text.
    ...
    recent_turns  : list of {"role": "user"|"iris", "content": str} — the window.
    user_msg      : the user's new message.
    iris          : optional tool wiring {"server", "can_use_tool"}; None disables
                    mid-chat jobs via the native tools.
    """
    _warn_if_metered()
    try:
        return asyncio.run(asyncio.wait_for(
            _converse_async(system, summary, recent_turns, user_msg, iris, model),
            CHAT_TIMEOUT_SEC))
