"""Durable, provider-agnostic state for Iris — the source of truth on disk.

Two things live through this module:

1. Iris's chat memory ("Talk to Iris"). Across sessions, Iris's ONLY long-term
...
2. The art engine's memory.json (a log of past design runs), through the same
...
def append_turn(state: dict[str, Any], role: str, content: str) -> None:
    """Append one turn. role is 'user' or 'iris' (kept provider-neutral)."""
