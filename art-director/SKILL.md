...
- **Palette.** A restrained, topic-appropriate base: a primary, a background, a text
  color, and **at most three** accents. No rainbow. The `signature_highlight` is the
  fixed house color `#FFD000` — it is added for you; never pick or change it.
- **Type system.** An editorial set with roles and weights: `display` (the big
  statement), `body` (the readable line), `caption` (the small label), and a modular
  `scale`. Few families, used with discipline.
- **Motion budget.** A `max_per_scene` (how many techniques a scene may carry,
  counting a non-cut transition as one) and an easing **philosophy**: stepped,
  deliberate motion — stepped-ease and a 12fps stutter feel over smooth tweens.
  Default budget is tight (2). You may go to 4 only with a reason; you never go to a
  fireworks show.
...
- **The signature beat.** Flag **exactly one** scene as the signature beat — the one
  moment that earns the animated `highlighter-FFD000` sweep. That effect appears on
  that scene **and no other.** (The #FFD000 *color* may still appear statically
