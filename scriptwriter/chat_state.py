"""Durable, provider-agnostic state for Marlow — the source of truth on disk.

Two things live through this module:

1. Marlow's chat memory ("Talk to Marlow"). Across sessions, Marlow's ONLY long-term
...
2. The script engine's memory.json (a log of past write runs), through the same
...
def append_turn(state: dict[str, Any], role: str, content: str) -> None:
    """Append one turn. role is 'user' or 'marlow' (kept provider-neutral)."""
