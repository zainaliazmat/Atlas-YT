"""Marlow's brain — one swappable chat() function, plus a converse() chat seam.
...
constant below (or set the MARLOW_LLM env var). Everything else is provider-agnostic.
...
- "gemini"   — Google Gemini (free tier). Activate with MARLOW_LLM=gemini and a
...
- "deepseek" — DeepSeek (OpenAI-compatible, raw requests). Activate with
  MARLOW_LLM=deepseek and a DEEPSEEK_API_KEY in .env. Wired but untested (no key
...
NOTE ON CHAT: the co-worker REPL ("Talk to Marlow", chat.py) does NOT use chat().
...
# ======================================================================
# Default (unset) = "claude" (subscription). Export MARLOW_LLM=gemini / deepseek to swap.
PROVIDER = os.environ.get("MARLOW_LLM", "claude").strip().lower()

# Per-use Claude models (subscription). Scriptwriting wants strong narrative
# reasoning; chat is fine on a fast Sonnet-class brain and keeps the heavier rate
# limit free.
CLAUDE_MODEL = "claude-opus-4-8"       # scriptwriting brain — Claude Opus 4.8 (full slug; no bare aliases)
...
# Alternative-provider models (only used when MARLOW_LLM selects them).
    ...
    if _genai_model is None:
        ...
        if not key:
            raise RuntimeError(
                ...
                "https://aistudio.google.com -> Get API key), or unset MARLOW_LLM to "
...
def _chat_deepseek(system: str, user: str) -> str:
    ...
    if not key:
        raise RuntimeError(
            ...
            "-> API keys), or unset MARLOW_LLM to use the default Claude subscription brain."
        )
    r = requests.post(
...
# ======================================================================
# MULTI-TURN SEAM — converse() for "Talk to Marlow" (chat.py)
# ======================================================================
# Always runs on the Claude Agent SDK: the persona chat needs the SDK's in-process
# tool + can_use_tool approval gate (mid-chat scripting with a [y/N] prompt). It is
...
# survives a future brain swap. The optional `marlow` wiring is the tool+approval
# bundle; pass marlow=None to disable the native-tool job (marker fallback handles it).


def _render_turns(recent_turns) -> str:
    label = {"user": "User", "marlow": "Marlow"}
    return "\n".join(
...
async def _converse_async(system, summary, recent_turns, user_msg, marlow, model) -> str:
    from claude_agent_sdk import query, ClaudeAgentOptions
    from claude_agent_sdk.types import AssistantMessage, TextBlock, ResultMessage
    ...
    if marlow is not None:
        ...
        opts_kwargs["mcp_servers"] = {"marlow": marlow["server"]}
        opts_kwargs["can_use_tool"] = marlow["can_use_tool"]
...
def converse(system: str, summary: str, recent_turns, user_msg: str,
             *, marlow=None, model: str | None = None) -> str:
    """Multi-turn chat seam (Claude SDK). Returns Marlow's reply text.
    ...
    recent_turns  : list of {"role": "user"|"marlow", "content": str} — the window.
    user_msg      : the user's new message.
    marlow        : optional tool wiring {"server", "can_use_tool"}; None disables
                    mid-chat scripting via the native tool.
    """
    _warn_if_metered()
    try:
        return asyncio.run(asyncio.wait_for(
            _converse_async(system, summary, recent_turns, user_msg, marlow, model),
            CHAT_TIMEOUT_SEC))
