"""Durable, provider-agnostic state for Quill — the source of truth on disk.

Two things live through this module:

1. Quill's chat memory ("Talk to Quill"). Across sessions, Quill's ONLY long-term
...
2. The coach engine's memory.json (a log of past coaching runs), through the same
...
def append_turn(state: dict[str, Any], role: str, content: str) -> None:
    """Append one turn. role is 'user' or 'quill' (kept provider-neutral)."""
