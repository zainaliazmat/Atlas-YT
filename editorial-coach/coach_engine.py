"""Quill — the Editorial Coach engine. Pure + injectable.

Given a diagnosed quality target (band + the direction to move it, decided by the
CEO-owned rubric — NOT by Quill), Quill authors a SOFT-TIER coaching addendum: a
few imperative sentences appended to a content specialist's persona/prompt to move
the metric into band on the next draft, WITHOUT regressing the named sibling
properties. Quill never reads the rubric, never writes files, never decides
...
DOMAIN = "editorial"
# pre-production / content stages this coach owns (must match atlas diagnose stages)
COACHED_STAGES = ("research", "script", "factcheck", "assets")

COACHING_PHILOSOPHY = (
    "You are Quill, an expert EDITORIAL coach for a video studio. You coach the "
    "content specialists — the researcher, the scriptwriter, the fact-check framing, "
    "and asset relevance. You think in terms of hook strength, one-idea-per-scene "
    "clarity, narrative arc, claim density and support, CTA, and whether visuals are "
    "RELEVANT to the narration. You give crisp, concrete, imperative coaching that a "
    "writer can act on in the next draft. You NEVER invent new goals beyond the metric "
    "named, and you NEVER let a fix break a sibling property you're told to preserve."
)
...
def _default_search(query: str, max_results: int = 5) -> list:
    ...
    import search  # editorial-coach/search.py (copied from Sage's proven seam)
...
def _research_query(band_id: str) -> str:
    ...
    return f"{DOMAIN} best practice: how to improve {prop} in a short explainer video script"
    ...
    if chat_fn is not None:
        try:
            sys_p = (COACHING_PHILOSOPHY + "\n\nYou are reviewing web findings for CURRENT "
                     ...
                     "anything you can't act on in a script.")
            ...
            pass
    # deterministic fallback: the raw titles are the (weak) hypotheses
...
def _rule_addendum(band_id: str, direction: str, preserve: str) -> str:
    return _wrap(band_id, (
        f"Your last output missed the editorial quality band for **{band_id}**: "
        f"{direction}. Revise to land inside the band on the next draft without "
